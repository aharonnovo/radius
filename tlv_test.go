...
func serverHandler(w radius.ResponseWriter, r *radius.Request) error {
	lookupRes, lookupErr := testtlv.TestStruct_Lookup(r.Packet)
	if lookupErr != nil {
		return lookupErr
	} else if !reflect.DeepEqual(lookupRes, expectedResults[0]) {
	...
	if getsErr != nil {
		return getsErr
	} else if !reflect.DeepEqual(getsRes, expectedResults) {
